 ...
 * 
 * Format:
 * - Fixed header: 13 bytes (version, type, ttl, timestamp, flags, payload_length)
...
export function truncatePeerId(peerId: PeerId): Uint8Array {
    ...
    return fullBytes.slice(0, SENDER_ID_SIZE);
}
...
function readUInt64BE(buffer: Uint8Array, offset: number): number {
    const high = (buffer[offset] << 24) |
        (buffer[offset + 1] << 16) |
        (buffer[offset + 2] << 8) |
        buffer[offset + 3];
    const low = (buffer[offset + 4] << 24) |
        (buffer[offset + 5] << 16) |
        (buffer[offset + 6] << 8) |
        buffer[offset + 7];
