...
export const useWalletBalances = () => {
  ...
  const fetchSolBalance = useCallback(async (pubKey: PublicKey): Promise<number> => {
    try {
      console.log('[useWalletBalances] 📡 Using Devnet RPC:', DEVNET_RPC);
      console.log('[useWalletBalances] 🔍 Fetching SOL balance for:', pubKey.toBase58());
      const balance = await connection.getBalance(pubKey);
      const solBalance = balance / LAMPORTS_PER_SOL;
      console.log('[useWalletBalances] ✅ SOL Balance (raw):', balance, 'lamports');
      console.log('[useWalletBalances] ✅ SOL Balance:', solBalance, 'SOL');
      return solBalance;
    } catch (error) {
      console.error('[useWalletBalances] ❌ Error fetching SOL balance:', error);
      return 0;
    }
  }, []);
  ...
  const fetchUSDCBalance = useCallback(async (pubKey: PublicKey): Promise<number> => {
    try {
      console.log('[useWalletBalances] 🔍 Fetching USDC balance for:', pubKey.toBase58());
      ...
      console.log('[useWalletBalances] 📊 USDC token accounts found:', tokenAccounts.value.length);
      ...
      console.error('[useWalletBalances] ❌ Error fetching USDC balance:', error);
      return 0;
    }
  }, []);
  ...
  const fetchZECBalance = useCallback(async (_pubKey: PublicKey): Promise<number> => {
    try {
      ...
      console.log('[useWalletBalances] ⚠️ ZEC balance fetching not yet implemented');
      return 0;
    } catch (error) {
      console.error('[useWalletBalances] ❌ Error fetching ZEC balance:', error);
      return 0;
    }
  }, []);
  ...
  const fetchBalances = useCallback(async (pubKey: PublicKey) => {
    ...
    console.log('[useWalletBalances] 🚀 Starting balance fetch from Devnet...');
    ...
    try {
      ...
      console.log('[useWalletBalances] 📊 Setting balances state:', newBalances);
