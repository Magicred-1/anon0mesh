...
impl<'info> DelegateEscrows<'info> {
    pub fn cpi_delegate_escrows(&self, amount: u64) -> Result<()> {
        // This is a placeholder for the actual CPI call to MagicBlock PER program
        // In a real implementation, you would call the MagicBlock PER program's delegate_escrows instruction
        // For now, we'll just log that the call would happen
        msg!("Delegate escrows CPI call would happen here with amount: {}", amount);
        Ok(())
    }
}
...
/// 4. `[writable]` source: [AccountInfo] Source account for delegation
/// 5. `[]` mint: [Mint] The token mint.
/// 6. `[writable]` destination: [AccountInfo] Destination account for delegation
...
pub fn handler(
    ...
    // Validate that the proxy transfer is in the integrated state
    require!(ctx.accounts.proxy_transfer.per_status == PerStatus::Integrated, ProxyTransferError::PerNotIntegrated);

    // Delegate escrows to MagicBlock PER
    ctx.accounts.cpi_delegate_escrows(ctx.accounts.proxy_transfer.amount)?;
