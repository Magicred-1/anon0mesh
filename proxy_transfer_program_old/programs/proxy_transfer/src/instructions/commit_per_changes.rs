...
impl<'info> CommitPerChanges<'info> {
	pub fn cpi_commit_per_changes(&self, amount: u64) -> Result<()> {
		// This is a placeholder for the actual CPI call to MagicBlock PER program
		// In a real implementation, you would call the MagicBlock PER program's commit_per_changes instruction
		// For now, we'll just log that the call would happen
		msg!("Commit PER changes CPI call would happen here with amount: {}", amount);
		Ok(())
	}
}
...
/// 4. `[writable]` source: [AccountInfo] Source account for commit
/// 5. `[]` mint: [Mint] The token mint.
/// 6. `[writable]` destination: [AccountInfo] Destination account for commit
...
pub fn handler(
    ...
    // Validate that the proxy transfer is in the delegated state
    require!(ctx.accounts.proxy_transfer.per_status == PerStatus::Delegated, ProxyTransferError::PerNotDelegated);

    // Commit PER changes to MagicBlock PER
    ctx.accounts.cpi_commit_per_changes(ctx.accounts.proxy_transfer.amount)?;

    // Update proxy transfer state to indicate committed changes
